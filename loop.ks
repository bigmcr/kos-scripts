		...
		IF tempChar = TERMINAL:INPUT:ENTER {
			...
			IF inputString <> "" {
				...
				ELSE {
					...
					IF inputString = "radialin" 				{SET useMySteer TO TRUE. SAS OFF. LOCK mySteer TO VCRS(SHIP:VELOCITY:ORBIT, VCRS(SHIP:VELOCITY:ORBIT, -SHIP:BODY:POSITION)). 	SET commandValid TO TRUE. SET loopMessage TO "Steering locked to radial in".} ELSE
					IF inputString = "radialout" 				{SET useMySteer TO TRUE. SAS OFF. LOCK mySteer TO -VCRS(SHIP:VELOCITY:ORBIT, VCRS(SHIP:VELOCITY:ORBIT, -SHIP:BODY:POSITION)). SET commandValid TO TRUE. SET loopMessage TO "Steering locked to radial out".} ELSE

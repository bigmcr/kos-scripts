...
UNTIL mode > 5 {
	...
	IF (TIME:SECONDS <> oldTime) {
		...
		SET x_r TO (SHIP:GEOPOSITION:TERRAINHEIGHT + SHIP:BODY:RADIUS) / SHIP:BODY:POSITION:MAG.
	...
	IF mode = 2 {
		...
		SET T_PID:SETPOINT TO minTimeToStop.
		IF (aboveGround < 10) advanceMode().
		SET myThrottle TO T_PID:UPDATE(TIME:SECONDS, timeToFall).
	...
	// Mode 3 - Maintain Vertical Speed at setpoint until height above ground is less than 5 meters
	// Note that the steering is limited to a pitch of 70 degrees at minimum. This cancels the last horizontal velocity
	IF mode = 3 {
	...
	// Mode 4 - Vertical Drop to the ground - use RCS for stabilization
	IF mode = 4 {
		RCS ON.
